...
export class FundedProposalRepository {
    ...
    async createFundedProposal(userId: string, data: SubmitFundedDto) {
        return await this.dbService.db.transaction(async (tx) => {
            ...
            const [proposal] = await tx.insert(schema.proposals)
                .values({
                    createdBy: userId,
                    title: data.title,
            ...
            const [version] = await tx.insert(schema.proposalVersions)
                .values({
            ...
            let totalAmount = 0;
            data.budgetItems.forEach(item => totalAmount += Number(item.requestedAmount));

            const [budgetRequest] = await tx.insert(schema.budgetRequests)
                .values({
                    proposalId: proposal.id,
                    requestedBy: userId,
    ...
    async getPendingProposalsForRole(roleName: string) {
        const pendingQuery = await this.dbService.db.select({
            proposal: schema.proposals,
            approval: schema.proposalApprovals,
            budget: schema.budgetRequests,
        })
            .from(schema.proposals)
            .innerJoin(schema.proposalApprovals, eq(schema.proposals.id, schema.proposalApprovals.proposalId))
            .leftJoin(schema.budgetRequests, eq(schema.proposals.id, schema.budgetRequests.proposalId))
            .where(
                and(
     ...
     */
    async assignEvaluators(proposalId: string, assignedById: string, evaluatorIds: string[]) {
        return await this.dbService.db.transaction(async (tx) => {
            ...
            const inserted = await tx.insert(schema.evaluatorAssignments).values(assignmentValues).returning();
     ...
     */
    async updateApprovalDecision(approvalId: string, data: ReviewFundedDto, approverUserId: string) {
        return await this.dbService.db.update(schema.proposalApprovals)
            .set({
