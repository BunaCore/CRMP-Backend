...
export class FundedWorkflowService {
  ...
  async processReview(
    ...
    const currentApproval =
      await this.dbService.db.query.proposalApprovals.findFirst({
        where: and(
    ...
    if (currentApproval.approverRole !== userRole) {
      throw new ForbiddenException(
        `Access denied. This step requires the ${currentApproval.approverRole} role.`,
      );
    }
    ...
    if (
      currentApproval.approverRole === 'FINANCE' &&
    ...
    if (data.decision === 'Rejected' || data.decision === 'Needs_Revision') {
      ...
      return {
        message: `Proposal marked as ${data.decision}. Workflow halted.`,
      };
    }
    ...
    if (data.decision === 'Accepted') {
      ...
      if (currentApproval.approverRole === 'EVALUATOR') {
        const parallelSteps =
          await this.dbService.db.query.proposalApprovals.findMany({
            where: and(
              eq(schema.proposalApprovals.proposalId, proposalId),
        ...
        const allAccepted = parallelSteps.every(
          (s) =>
        ...
        if (!allAccepted) {
          ...
          return {
            message: `Decision Accepted recorded. Waiting for other evaluators to finish.`,
          };
        }
      }
      ...
      if (nextStepResult === null || nextStepResult === 'AC_SKIPPED') {
        ...
        const activatedProject = await this.proposalRepo.activateFundedProject(
          proposalId,
          userId,
        );
        return {
      ...
      return {
        message: `Proposal advanced to routing step ${currentApproval.stepOrder + 1}.`,
      };
    }
  }
}
