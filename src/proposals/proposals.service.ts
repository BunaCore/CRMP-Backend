    ...
    async create(user: any, dto: CreateProposalDto, file: Express.Multer.File) {
        try {
            return await this.drizzle.db.transaction(async (tx) => {
                ...
                const [proposalFile] = await tx.insert(schema.proposalFiles).values({
                ...
                const [version] = await tx.insert(schema.proposalVersions).values({
                    proposalId: proposal.id,
                    createdBy: user.id,
                    versionNumber: 1,
                    isCurrent: true,
                ...
                const totalAmount = dto.budget.reduce((sum, item) => sum + Number(item.amount), 0);
                const [budgetRequest] = await tx.insert(schema.budgetRequests).values({
                    proposalId: proposal.id,
                    requestedBy: user.id,
                ...
                return {
                    id: proposal.id,
                    status: 'Submitted',
                    message: 'Proposal recorded successfully. All relations and budget items synchronized.',
                };
            });
        } catch (error) {
            console.error('Core Transaction Failed:', error);
            throw new InternalServerErrorException('Database synchronization failed during proposal recording.');
        }
    }
}
