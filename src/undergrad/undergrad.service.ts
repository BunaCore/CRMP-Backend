...
export class UndergradService {
    ...
    async makeDecision(
        ...
        const pendingApproval = await this.repo.findPendingCoordinatorApproval(proposalId);
        if (!pendingApproval) {
        ...
        const statusMap: Record<CoordinatorDecision, {
        ...
        const { newStatus, unlockWorkspace, notificationType } = statusMap[dto.decision];
    ...
    async assignAdvisor(
        ...
        const existing = await this.repo.findExistingAssignment(proposalId, dto.advisorUserId);
        if (existing) {
            throw new ConflictException(
