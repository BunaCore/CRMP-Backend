import { pgTable, pgEnum, uuid, integer, numeric, text, varchar, timestamp } from 'drizzle-orm/pg-core';
import { users } from './user';
import { proposals } from './proposals';
import { projects } from './project';

export const fundReleaseStatusEnum = pgEnum('fund_release_status', ['Pending', 'Released', 'Cancelled']);
export const installmentTriggerEnum = pgEnum('installment_trigger', ['Auto', 'Manual']);
...
export const budgetRequestItems = pgTable('budget_request_items', {
    id: uuid('id').primaryKey().defaultRandom(),
    budgetRequestId: uuid('budget_request_id').notNull().references(() => budgetRequests.id, { onDelete: 'cascade' }),
...
export const budgetLedger = pgTable('budget_ledger', {
    id: uuid('id').primaryKey().defaultRandom(),
    budgetRequestId: uuid('budget_request_id').notNull().references(() => budgetRequests.id),
    installmentId: uuid('installment_id').references(() => budgetInstallments.id),
    amountReleased: numeric('amount_released', { precision: 15, scale: 2 }).notNull(),
